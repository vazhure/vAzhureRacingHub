...
namespace KunosPlugin
{
    internal class GameListener : VAzhureSharedMemoryClient
    {
        ...
        {
            GamePlugin runningGame = games.FirstOrDefault(x => x.IsRunning && x.Enabled);

            // Сброс состояния при выключении игры или смене активной
            if (runningGame == null)
            {
            ...
            if (m_RunningGame != runningGame)
            {
                ...
                m_packetID = -1; // Сбрасываем ID при смене игры
                runningGame.NotifyGameState();
            }

            if (runningGame.gameID == GameID.ACEVO)
            {
                GameListenerEvo.Process(runningGame, ref m_packetID, out bool hasNewData);
                if (hasNewData) runningGame.NotifyTelemetry();
            ...
            try
            {
                ...
                using (var viewStreamGraphics = memoryFileGraphics.CreateViewStream(0L, dataGraphics.Length, MemoryMappedFileAccess.ReadWrite))
                {
                    ...
                    {
                        viewStreamStatic.Read(dataStatic, 0, dataStatic.Length);
                    ...
                    {
                        viewStreamGraphics.Read(dataGraphics, 0, dataGraphics.Length);
                        ...
                        {
                            case AC_SESSION_TYPE.AC_PRACTICE: SessionInfo.SessionState = "Practice"; break;
                            case AC_SESSION_TYPE.AC_HOTLAP: SessionInfo.SessionState = "Hot Lap"; break;
                            case AC_SESSION_TYPE.AC_DRAG: SessionInfo.SessionState = "Drag"; break;
                            case AC_SESSION_TYPE.AC_QUALIFY: SessionInfo.SessionState = "Qualifying"; break;
                            case AC_SESSION_TYPE.AC_RACE: SessionInfo.SessionState = "Race"; break;
                            case AC_SESSION_TYPE.AC_TIME_ATTACK: SessionInfo.SessionState = "Time attack"; break;
                        ...
                        {
                            SessionInfo.Flag = " ";
                            if (graphics.GlobalGreen > 0) { SessionInfo.Flag = "Green"; CarData.Flags = CarData.Flags == TelemetryFlags.FlagNone ? TelemetryFlags.FlagGreen : CarData.Flags; }
                            else if (graphics.GlobalYellow > 0) { SessionInfo.Flag = "Yellow"; CarData.Flags = CarData.Flags == TelemetryFlags.FlagNone ? TelemetryFlags.FlagYellow : CarData.Flags; }
                            else if (graphics.GlobalRed > 0) { SessionInfo.Flag = "Red"; CarData.Flags = CarData.Flags == TelemetryFlags.FlagNone ? TelemetryFlags.FlagPenalty : CarData.Flags; }
                            else if (graphics.GlobalWhite > 0) { SessionInfo.Flag = "White"; CarData.Flags = CarData.Flags == TelemetryFlags.FlagNone ? TelemetryFlags.FlagWhite : CarData.Flags; }
                            else if (graphics.GlobalChequered > 0) { SessionInfo.Flag = "Chequered"; CarData.Flags = CarData.Flags == TelemetryFlags.FlagNone ? TelemetryFlags.FlagChequered : CarData.Flags; }
                        }
                        SessionInfo.FinishStatus = graphics.GlobalChequered > 0 ? "Finished" : " ";
                    }

                    viewStreamPhysic.Read(dataPhysics, 0, dataPhysics.Length);
                    ...
                    if (physics.packetId != m_packetID)
                    {
                        ...
                        {
                            new AMTireData { BrakeTemperature = physics.brakeTemp[0], Pressure = physics.wheelsPressure[0] * 6.89476, Wear = 1.0f - physics.tyreWear[0] / 100f, Temperature = physics.tyreTempM[0] > 0 ? new double[] { physics.tyreTempI[0], physics.tyreTempM[0], physics.tyreTempO[0] } : new double[] { physics.tyreCoreTemperature[0], physics.tyreCoreTemperature[0], physics.tyreCoreTemperature[0] }, Compound = sTireCompound },
                            new AMTireData { BrakeTemperature = physics.brakeTemp[1], Pressure = physics.wheelsPressure[1] * 6.89476, Wear = 1.0f - physics.tyreWear[1] / 100f, Temperature = physics.tyreTempM[1] > 0 ? new double[] { physics.tyreTempI[1], physics.tyreTempM[1], physics.tyreTempO[1] } : new double[] { physics.tyreCoreTemperature[1], physics.tyreCoreTemperature[1], physics.tyreCoreTemperature[1] }, Compound = sTireCompound },
                            new AMTireData { BrakeTemperature = physics.brakeTemp[2], Pressure = physics.wheelsPressure[2] * 6.89476, Wear = 1.0f - physics.tyreWear[2] / 100f, Temperature = physics.tyreTempM[2] > 0 ? new double[] { physics.tyreTempI[2], physics.tyreTempM[2], physics.tyreTempO[2] } : new double[] { physics.tyreCoreTemperature[2], physics.tyreCoreTemperature[2], physics.tyreCoreTemperature[2] }, Compound = sTireCompound },
                            new AMTireData { BrakeTemperature = physics.brakeTemp[3], Pressure = physics.wheelsPressure[3] * 6.89476, Wear = 1.0f - physics.tyreWear[3] / 100f, Temperature = physics.tyreTempM[3] > 0 ? new double[] { physics.tyreTempI[3], physics.tyreTempM[3], physics.tyreTempO[3] } : new double[] { physics.tyreCoreTemperature[3], physics.tyreCoreTemperature[3], physics.tyreCoreTemperature[3] }, Compound = sTireCompound },
                        };
